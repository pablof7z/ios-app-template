...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "AppTemplate", category: "OpenRouterKeyValidationService")
...
struct OpenRouterKeyValidationService: Sendable {
    ...
    func validate(apiKey: String) async throws -> OpenRouterKeyInfo {
        ...
        default:
            logger.warning("OpenRouter auth/key returned \(http.statusCode, privacy: .public)")
            throw ValidationError.serverError(statusCode: http.statusCode)
        }

        do {
            let dto = try JSONDecoder().decode(ORAuthKeyResponse.self, from: data)
            ...
            logger.error("OpenRouter auth/key decode failed: \(error, privacy: .public)")
