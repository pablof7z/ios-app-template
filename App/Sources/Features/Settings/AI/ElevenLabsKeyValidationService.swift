...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "AppTemplate", category: "ElevenLabsKeyValidationService")
...
struct ElevenLabsKeyValidationService: Sendable {
    ...
    func validate(apiKey: String) async throws -> ElevenLabsKeyInfo {
        ...
        default:
            logger.warning("ElevenLabs /v1/user returned \(http.statusCode, privacy: .public)")
            throw ElevenLabsValidationError.serverError(statusCode: http.statusCode)
        }

        do {
            let dto = try JSONDecoder().decode(ELUserResponse.self, from: data)
            ...
            logger.error("ElevenLabs /v1/user decode failed: \(error, privacy: .public)")
