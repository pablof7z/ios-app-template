...
struct AgentHubView: View {
    ...
    private var nostrSection: some View {
        Section {
            Toggle("Enable Nostr Agent", isOn: $settings.nostrEnabled)
            if settings.nostrEnabled {
    ...
    private var agentSection: some View {
        Section {
            navRow(title: "Identity & Profile", icon: "person.crop.circle", tint: .purple) {
                AgentIdentityView()
            }
            navRow(title: "Friends", icon: "person.2.fill", tint: .blue, badge: store.state.friends.count) {
                AgentFriendsView()
            }
            navRow(
                title: "Whitelist",
            ...
            navRow(title: "Blocked", icon: "nosign", tint: .red, badge: store.state.nostrBlockedPubkeys.count) {
                AgentBlockedView()
            }
            navRow(title: "Memories", icon: "brain", tint: .purple, badge: store.activeMemories.count) {
                AgentMemoriesView()
            }
        } header: {
            Text("Agent Screens")
            ...
            HStack {
                Label {
                    ...
                    Image(systemName: icon)
                        ...
                        .symbolEffect(.bounce, value: animated ? badge : 0)
                }
                Spacer()
                if badge > 0 {
                    StatBadge(value: badge, label: nil, color: badgeColor)
                }
            }
        }
    }

    // MARK: - OpenRouter status
